...
class SudokuDataset(GSM8KDataset):
    ...
    def __init__(
        ...
        if split == "test":
            self.sudoku_file_path = os.path.join(dir_path, "../dataset", "4x4_test_sudoku.csv")
        else:
            self.sudoku_file_path = os.path.join(dir_path, "../dataset", "4x4_sudoku_unique_puzzles.csv")
        super().__init__(
            tokenizer, 
            num_examples, 
            add_reasoning, 
    ...
    def load_test_dataset(self):
        ...
        # Keep only the first 1000 samples, this is kept same with diffu-grpo, the grpo baseline.
        if self.split == "train":
            df = df.head(1000)
    ...
    def __getitem__(self, idx):
        ...
        prompt = self.create_prompt(question, None)
        return prompt, question, solution  # prompt, question, answer